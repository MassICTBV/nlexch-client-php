...
/**
 *
 * A Simple PHP NLexch API Client
 *
 * @version 0.1.0
 */

class NLexchClientException extends Exception {}
...
class NLexchClient
{
  ...
  public function __construct($options=array()) {
    $default_options = array(
      'api_host'    => "https://www.nlexch.com/",
      'access_key'  => "Your Access Key",
      'secret_key'  => "Your Secret Key",
      'timeout_sec' => 30,
      'user_agent'  => "NLexch API Client/0.1.0",
    );
  ...
  public function get($path, $params=null) {
    return $this->nlexch_api_req($path, $params);
  }

  public function post($path, $params=null) {
    return $this->nlexch_api_req($path, $params, 'POST');
  }
  ...
  private function nlexch_api_req($path, $params=null, $canonical_verb='GET') {
    ...
    if (empty($content)) {
      throw new NLexchClientException("Content is empty.");
    }
    $obj = json_decode($content, 1);
    if (empty($obj)) {
      throw new NLexchClientException("JSON decode failed, content: " .$content);
    }
    return $obj;
  ...
  private function curl_get_contents($url, $post_params=null, $timeout_sec=5, $user_agent) {
    ...
    curl_close($ch);
    if ($curl_errno > 0) {
      throw new NLexchClientException("cURL Error ($curl_errno): $curl_error, url: {$url}");
    }
    return $r;
  }
